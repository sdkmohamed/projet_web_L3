...
<body>
<!-- Code HTML pour afficher les détails de l'étape -->
<div class="etape-container">
    <?php if (!empty($etape)) : ?>
        <h1 style="color: #004080; margin-bottom: 20px;">Détails de l'étape 🎮</h1>
        <div>
            ...
            <?php if ($lecode) : ?>
                <?php if ($reponseCorrecte) : ?>
                        <form action="<?php echo base_url("index.php/scenario/franchir_etape/{$lecode}/{$niveau}"); ?>" method="post">
                            <?= csrf_field() ?>
                    ...
                    <?php else : ?>
                        <form action="<?php echo base_url("index.php/scenario/franchir_etape/{$code}/{$niveau}"); ?>" method="post">
                            <?= csrf_field() ?>
                            <label for="reponse">Votre réponse :</label>
                            <textarea class="reponse-input" name="reponse" placeholder="Votre réponse"></textarea>
                            <input type="submit" name="submit" value="Valider ✅" style="background-color: #004080; color: #ffffff; padding: 10px;">
                    ...
                    <?php endif; ?>
            <?php else : ?>
                <form action="<?php echo base_url("index.php/scenario/fin_etape/{$code}/{$lcode}/{$concat}/{$niveau}"); ?>" method="post">
                    <?= csrf_field() ?>
                    <label for="reponse">Votre réponse :</label>
                    <textarea class="reponse-input" name="reponse" placeholder="Votre réponse"></textarea>
                    <input type="submit" name="submit" value="Valider ✅" style="background-color: #004080; color: #ffffff; padding: 10px;">
    ...
    <?php else : ?>
        <p style="color: #004080;">Les détails de l'étape n'ont pas été trouvés.</p>
    <?php endif; ?>
